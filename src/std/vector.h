...
#define DEFINE_VECTOR(TypeName, T, AllocType, AllocPrefix)              \
  ...
  {                                                                     \
     ...
     * 或 ALLOC(BUMP, alloc, ...)                                      \
     */                                                                 \
    TypeName *self = (TypeName *)ALLOC(                                 \
      AllocPrefix, alloc, LAYOUT_OF(TypeName));                         \
  ...
  /* --- 3. 核心 API (Capacity) --- */                                  \
                                                                        \
  static inline void TypeName##_reserve_to(TypeName *self,              \
                                           usize new_cap)               \
  {                                                                     \
    if (new_cap <= self->cap)                                           \
      return;                                                           \
    ...
    Layout new_layout = LAYOUT_OF_ARRAY(T, new_cap);                    \
                                                                        \
    /* * 静态分发：REALLOC(SYSTEM, ...) */                              \
    self->data = (T *)REALLOC(AllocPrefix,                              \
                              self->alloc_state,                        \
                              self->data,                               \
                              old_layout,                               \
                              new_layout);                              \
    ...
    self->cap = new_cap;                                                \
                                                                        ...
                                                                        \
  static inline void TypeName##_reserve_more(                           \
    TypeName *self, usize additional)                                   \
  {                                                                     \
    if (self->len + additional <= self->cap)                            \
      return;                                                           \
    usize required_cap = self->len + additional;                        \
    usize new_cap =                                                     \
      (self->cap == 0) ? 8 : (self->cap * 2);                           \
  ...
  /* --- 4. 核心 API (Access) --- */                                    \
                                                                        \
  static inline void TypeName##_push(TypeName *self,                    \
                                     T element)                         \
  {                                                                     \
  ...
  {                                                                     \
    return self->data;                                                  \
                                                                        ...
                                                                        \
  static inline const T *TypeName##_as_const_ptr(                       \
    const TypeName *self)                                               \
  {                                                                     \
    return self->data;                                                  \
  }
