...
#define DEFINE_BITSET(TypeName, AllocType, AllocPrefix)                                            \
  ...
  {                                                                                                \
    ...
    AllocType *alloc_state;                                                                        \
                                                                                                   ...
                                                                                                   \
  static inline TypeName *TypeName##_create(AllocType *alloc, usize num_bits)                      \
  {                                                                                                \
    asrt(alloc != NULL);                                                                           \
    usize num_words = bitset_words_for_bits(num_bits);                                             \
    Layout word_layout = LAYOUT_OF_ARRAY(u64, num_words);                                          \
                                                                                                   \
    TypeName *bs = (TypeName *)oexpect(sys_malloc(sizeof(TypeName)),                               \
  ...
  {                                                                                                \
    ...
    sys_free(bs);                                                                                  \
                                                                                                   ...
                                                                                                   \
  static inline TypeName *TypeName##_create_all(AllocType *alloc, usize num_bits)                  \
  {                                                                                                \
    asrt(alloc != NULL);                                                                           \
    usize num_words = bitset_words_for_bits(num_bits);                                             \
    Layout word_layout = LAYOUT_OF_ARRAY(u64, num_words);                                          \
    ...
    return bs;                                                                                     \
                                                                                                   ...
                                                                                                   \
  static inline void TypeName##_set(TypeName *bs, usize bit)                                       \
  {                                                                                                \
    asrt(bs != NULL && bs->words != NULL);                                                         \
    asrt_msg(bit < bs->num_bits, "Bitset_set: index out of bounds");                               \
    bs->words[bitset_bit_index(bit)] |= bitset_bit_mask(bit);                                      \
  }                                                                                                \
                                                                                                   \
  static inline void TypeName##_clear(TypeName *bs, usize bit)                                     \
  {                                                                                                \
    asrt(bs != NULL && bs->words != NULL);                                                         \
    asrt_msg(bit < bs->num_bits, "Bitset_clear: index out of bounds");                             \
    bs->words[bitset_bit_index(bit)] &= ~bitset_bit_mask(bit);                                     \
  }                                                                                                \
                                                                                                   \
  static inline bool TypeName##_test(const TypeName *bs, usize bit)                                \
  {                                                                                                \
    asrt(bs != NULL && bs->words != NULL);                                                         \
    asrt_msg(bit < bs->num_bits, "Bitset_test: index out of bounds");                              \
    return (bs->words[bitset_bit_index(bit)] & bitset_bit_mask(bit)) != 0;                         \
  }                                                                                                \
                                                                                                   \
  ...
  {                                                                                                \
    ...
    memset(bs->words, 0, bs->num_words * sizeof(u64));                                             \
                                                                                                   ...
                                                                                                   \
  static inline bool TypeName##_equals(const TypeName *bs1, const TypeName *bs2)                   \
  {                                                                                                \
    asrt(bs1 != NULL && bs1->words != NULL);                                                       \
    asrt(bs2 != NULL && bs2->words != NULL);                                                       \
    asrt_msg(bs1->num_bits == bs2->num_bits, "Bitset_equals: mismatched sizes");                   \
    if (bs1->num_words == 0)                                                                       \
      return true;                                                                                 \
    return memcmp(bs1->words, bs2->words, bs1->num_words * sizeof(u64)) == 0;                      \
  }                                                                                                \
                                                                                                   \
  static inline void TypeName##_copy(TypeName *dest, const TypeName *src)                          \
  {                                                                                                \
    asrt(dest != NULL && dest->words != NULL);                                                     \
    asrt(src != NULL && src->words != NULL);                                                       \
    asrt_msg(dest->num_bits == src->num_bits, "Bitset_copy: mismatched sizes");                    \
    if (src->num_words == 0)                                                                       \
      return;                                                                                      \
    memcpy(dest->words, src->words, dest->num_words * sizeof(u64));                                \
  }                                                                                                \
                                                                                                   \
  static inline void TypeName##_intersect(                                                         \
    TypeName *dest, const TypeName *src1, const TypeName *src2)                                    \
  {                                                                                                \
    asrt(dest->num_bits == src1->num_bits && dest->num_bits == src2->num_bits);                    \
    ...
    {                                                                                              \
      dest->words[i] = src1->words[i] & src2->words[i];                                            \
                                                                                                   ...
                                                                                                   \
  static inline void TypeName##_union(TypeName *dest, const TypeName *src1, const TypeName *src2)  \
  {                                                                                                \
    asrt(dest->num_bits == src1->num_bits && dest->num_bits == src2->num_bits);                    \
    for (usize i = 0; i < dest->num_words; ++i)                                                    \
    {                                                                                              \
  ...
  {                                                                                                \
    ...
    {                                                                                              \
      dest->words[i] = src1->words[i] & ~src2->words[i];                                           \
                                                                                                   ...
                                                                                                   \
  static inline usize TypeName##_count_slow(const TypeName *bs)                                    \
  {                                                                                                \
    asrt(bs != NULL && bs->words != NULL);                                                         \
    usize count = 0;                                                                               \
...
#define bs_create(alloc, num_bits)                                                                 \
  _Generic((alloc), SystemAlloc *: sbitset_create, Bump *: bbitset_create)(alloc, num_bits)

#define bs_destroy(self)                                                                           \
  _Generic((self), sbitset *: sbitset_destroy, bbitset *: bbitset_destroy)(self)

#define bs_create_all(alloc, num_bits)                                                             \
  _Generic((alloc), SystemAlloc *: sbitset_create_all, Bump *: bbitset_create_all)(alloc, num_bits)

#define bs_set(self, bit)                                                                          \
  _Generic((self), sbitset *: sbitset_set, bbitset *: bbitset_set)(self, bit)

#define bs_clear(self, bit)                                                                        \
  _Generic((self), sbitset *: sbitset_clear, bbitset *: bbitset_clear)(self, bit)
...
#define bs_set_all(self)                                                                           \
  _Generic((self), sbitset *: sbitset_set_all, bbitset *: bbitset_set_all)(self)

#define bs_clear_all(self)                                                                         \
  _Generic((self), sbitset *: sbitset_clear_all, bbitset *: bbitset_clear_all)(self)
...
#define bs_copy(dest, src)                                                                         \
  _Generic((dest), sbitset *: sbitset_copy, bbitset *: bbitset_copy)(dest, src)

#define bs_intersect(dest, src1, src2)                                                             \
  _Generic((dest), sbitset *: sbitset_intersect, bbitset *: bbitset_intersect)(dest, src1, src2)

#define bs_union(dest, src1, src2)                                                                 \
  _Generic((dest), sbitset *: sbitset_union, bbitset *: bbitset_union)(dest, src1, src2)

#define bs_difference(dest, src1, src2)                                                            \
  _Generic((dest), sbitset *: sbitset_difference, bbitset *: bbitset_difference)(dest, src1, src2)
