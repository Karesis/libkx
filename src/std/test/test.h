  ...
  {                                                                                                \
    ...
    {                                                                                              \
      ...
      format_to_file(stderr,                                                                       \
                     ...
                     __LINE__,                                                                     \
                     reset());                                                                     \
      format_to_file(stderr, "           Condition: {}\n", #cond);                                 \
      format_to_file(stderr, "           Message:   ");                                            \
      /* (再次调用 vformat 来打印用户消息) */                                                      \
      format_to_file(stderr, fmt __VA_OPT__(, ) __VA_ARGS__);                                      \
      format_to_file(stderr, "\n");                                                                \
    }                                                                                              \
  } while (0)
 ...
 * 失败时，打印错误并 *中止* (panic)。
 */
#define TEST_ASSERT_FATAL(cond, fmt, ...) asrt_msg(cond, fmt __VA_OPT__(, ) __VA_ARGS__)
...
#define SUITE_END()                                                                                \
  ...
  {                                                                                                \
    ...
    {                                                                                              \
      ...
      format_to_file(stderr, "{}*** Suite FAILED ***{}\n", fg(rgb(255, 80, 80)), reset());         \
    }                                                                                              \
  } while (0)
...
#define TEST_SUMMARY()                                                                             \
  ...
  {                                                                                                \
    format_to_file(stdout, "\n============================\n");                                    \
    format_to_file(stdout, "  Total Suites Run:    {}\n", g_kx_test_total_run);                    \
    format_to_file(stdout,                                                                         \
                   "  Total Suites Failed: {}{}{}\n",                                              \
