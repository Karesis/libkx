...
#define rexpect(res, msg)                                  \
  ({                                                       \
    ...
    {                                                      \
      panic("Failed expectation (expected Ok): {}",        \
            (msg));                                        \
    }                                                      \
...
#define rexpect_err(res, msg)                              \
  ({                                                       \
    ...
    if (ris_ok(__res_tmp))                                 \
    {                                                      \
      panic("Failed expectation (expected Err): {}",       \
            (msg));                                        \
    }                                                      \
...
#define runwrap_or(res, default_val)                       \
  ({                                                       \
    __auto_type __res_tmp = (res);                         \
    ris_ok(__res_tmp) ? __res_tmp.value.ok                 \
                      : (default_val);                     \
  })
...
#define rmap(out_ok_type, err_type, res_in, var, ...)      \
  ({                                                       \
    ...
    (ris_err(__res_tmp))                                   \
      ? Err(out_ok_type, err_type, __res_tmp.value.err)    \
      : ({                                                 \
          typeof(__res_tmp.value.ok) var =                 \
            __res_tmp.value.ok;                            \
...
#define rmap_err(ok_type, out_err_type, res_in, var, ...)  \
  ({                                                       \
    __auto_type __res_tmp = (res_in);                      \
    (ris_ok(__res_tmp))                                    \
      ? Ok(ok_type, out_err_type, __res_tmp.value.ok)      \
      : ({                                                 \
          typeof(__res_tmp.value.err) var =                \
            __res_tmp.value.err;                           \
